...
void enqueuePriority(Pelanggan p){
    ...
    if(frontQ == nullptr || baru->data.prioritas < frontQ->data.prioritas){
        baru->next = frontQ;
        frontQ = baru;
    }
    else{
        Queue *bantu = frontQ;
        while(bantu->next != nullptr && bantu->next->data.prioritas <= baru->data.prioritas){
            bantu = bantu->next;
        }
...
void isiPCKosong(){
    for(int i = 0; i < MAX_PC; i++){
        if(!terisi[i] && frontQ != nullptr){
            ...
            pc[i] = p;
            terisi[i] = true;
            cout << p.nama << " masuk ke PC " << i + 1 << endl;
        }
    }
}
...
void masukAntrian(){
    ...
    cout << p->data.nama << " masuk antrian\n";
...
void tampilAntrian(){
    ...
    cout << "\n=== PRIORITY QUEUE ===\n";
    while(bantu != nullptr){
        cout << no++ << ". " << bantu->data.nama << " (" << bantu->data.status << ")\n";
        bantu = bantu->next;
    }
}
...
void selesaiMain(){
    ...
    cout << pc[nomor].nama << " selesai bermain\n";
    terisi[nomor] = false;
    ...
    if(frontQ != nullptr){
        ...
        pc[nomor] = p;
        terisi[nomor] = true;
        cout << p.nama << " masuk ke PC " << nomor + 1 << endl;
    }
}
...
int main(){
    ...
    do{
        ...
        switch(pilih){
            ...
            case 2: masukAntrian(); break;
            case 3: tampilPelanggan(); break;
            case 4: tampilAntrian(); break;
            case 5: tampilPC(); break;
            case 6: selesaiMain(); break;
